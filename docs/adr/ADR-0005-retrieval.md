...
方案：Milvus standalone（docker compose）单 collection `lingnan_photos`；BGE-M3 dense+sparse（COSINE 0.8 + IP 0.2，norm_score 归一）承担文本通道；CLIP ViT-B/32 承担图像通道；两通道 RRF 融合后交 rerank。嵌入与 Milvus 连接全部单例化。
